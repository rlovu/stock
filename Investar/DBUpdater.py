...
class DBUpdater:
    def __init__(self):
        ...
        self.conn = pymysql.connect(host='localhost', port=13306, user='eugene', passwd='eugene', db='stock', charset='utf8')
    ...
    def read_naver(self, code, company, pages_to_fetch):
        ...
        try:
            ...
            df['diff'] = df['diff'].str.replace(',', '')
            df['diff'] = df['diff'].str.replace('상승', '')
            df['diff'] = df['diff'].str.replace('보합', '')
            df['diff'] = df['diff'].str.replace('하락', '-')
            df['diff'] = df['diff'].str.replace('상한가', '')
            df['diff'] = df['diff'].str.replace('하한가', '')
            df = df.dropna()

            df[['close','open', 'high', 'low', 'diff', 'volume']] = df[['close','open', 'high', 'low', 'diff', 'volume']].astype('int')
